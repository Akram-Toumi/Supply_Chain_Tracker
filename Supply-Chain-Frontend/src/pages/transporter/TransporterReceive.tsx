...
const TransporterReceive = () => {
  ...
  const handleReceiveInTransit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!receiveLocation.trim() || receivingProductId === null) return;

    try {
      setReceiving(true);
      await receiveInTransit(receivingProductId, receiveLocation);
      setReceivingProductId(null);
      setReceiveLocation('');
      // Show success message
      alert(`Product #${receivingProductId} location updated to: ${receiveLocation}`);
    } catch (error) {
  ...
  const canReceiveInTransit = (product: any) => {
    ...
    <div className="space-y-6">
      ...
      {receivingProductId && (
        <div className="bg-[#1F2937] rounded-lg p-6">
          <h3 className="text-lg font-semibold text-white mb-4">Update Product #{receivingProductId} Location</h3>
          <p className="text-gray-400 text-sm mb-4">
            Update the current location of this product in transit. The product will remain in IN_TRANSIT state.
          </p>
          <form onSubmit={handleReceiveInTransit} className="space-y-4">
            <div>
      ...
      <div className="bg-[#1F2937] rounded-lg p-6">
        <h2 className="text-xl font-semibold text-white mb-4">Products in Transit</h2>
        {loading ? (
                ...
                {products.filter(product => product.currentState === 1).length === 0 ? (
                  <tr>
                    <td colSpan={6} className="text-center py-8 text-gray-400">
                      No products in transit found. Products will appear here after they are shipped by producers.
                    </td>
                  </tr>
                ) : (
                  products
...
export default TransporterReceive;
